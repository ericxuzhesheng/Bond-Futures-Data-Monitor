...
| 规则分类器（`rule-based-text-signal-v2`）| 未设置 API key 或 API 调用失败 | 覆盖 8 种 event_type 的关键词映射，编码研究人员的固定收益领域知识 |
...
| DR007 变化 | 衡量银行间资金松紧 |
| 国债期货日收益率 | 衡量期货价格动量 |
| 成交量变化 | 当日成交量相对前一交易日的百分比变化（从数据库历史计算） |
...
如需自定义路径，可复制 `.env.example` 为 `.env`。可选项：设置 `ANTHROPIC_API_KEY` 启用真实 LLM 文本分析。
