...
国债期货各品种收跌。TS 表现相对最强，日收益 -0.021%；TL 为 -0.189%。

10年国债收益率为 1.715%，较上一观测日变化 +0.8 bp。FDR007 为 1.390%，变化 -1.0 bp。央行公开市场当日净回笼 465 亿元。

模型中的方向性得分来自以下判断。10Y 收益率显著上行（60日分位 87%），压制国债期货。期货下跌且成交活跃度提高，量价配合偏消极。制造业 PMI 49.2 低于荣枯线，基本面偏弱支撑债市。


模型评分 **-2.50** · 规则置信指标 **87/100**
...
|---|---:|---:|---:|---:|
| 10Y国债收益率 | 1.715% | +0.8 bp | +0.3 bp | 35% |
| FDR007 | 1.390% | -1.0 bp | +1.0 bp | 30% |
| 期货平均收益 | -0.079% | -0.079% | +0.066% | 10% |
| 总成交量 | 299,497 | +6.5% | +2.7% | 80% |
| 总持仓量 | 832,661 | -3.1% | -5.5% | 5% |

*收益率与资金利率的变化以 bp 计，1 bp 等于 0.01 个百分点。分位只使用已有同口径观测，不足20条时按实际样本计算；无法比较时注明原因。期货5日栏为五次观测的复合收益，成交量和持仓量5日栏为相对五个交易观测前的变化率。*

## 期货表现

![国债期货累计表现](assets/2026-08-11_futures_20d.svg)

*以窗口首个观测日前的100为基准，逐日复合各品种收益率；横轴列出观测日期。*

| 合约 | 日收益 | 持仓变化 | 成交量/均值 | 持仓量/均值 | 量价状态 |
|---|---:|---:|---:|---:|---|
| TS | -0.021% | -5.11% | 1.00x | 0.93x | 价跌减仓 |
| TF | -0.061% | -2.42% | 1.05x | 0.94x | 价跌减仓 |
| T | -0.046% | -1.52% | 1.03x | 0.96x | 价跌减仓 |
| TL | -0.189% | -5.80% | 1.10x | 0.95x | 价跌减仓 |
...
| 合约 | 收盘价 | 日收益率 | 成交量 | 持仓量 |
|---|---:|---:|---:|---:|
| TS | 102.602 | -0.0214% | 39,329 | 82,206 |
| TF | 106.500 | -0.0610% | 66,795 | 207,487 |
| T | 109.430 | -0.0457% | 89,379 | 357,298 |
| TL | 115.920 | -0.1894% | 103,994 | 185,670 |
...
![当日与前一观测日国债收益率曲线](assets/2026-08-11_yield_curve.svg)

| 结构 | 当前 | 1日变化 | 5日变化 |
|---|---:|---:|---:|
| 2s10s | +46.2 bp | +0.4 bp | +1.0 bp |
| 10s30s | +45.4 bp | +0.3 bp | -1.6 bp |
| 2s5s10s蝶式 | +15.3 bp | +0.1 bp | +0.3 bp |
...
![资金利率历史观测](assets/2026-08-11_funding_20d.svg)
...
| FR007 | 1.420% | +0.0 bp |
| R007 | 1.645% | +4.2 bp |
| SHIBOR_7D | 1.394% | -0.6 bp |
| SHIBOR_ON | 1.358% | -1.9 bp |
...
![CPI、PPI 与制造业 PMI 历史](assets/2026-08-11_macro_history.svg)
...
**中报窗口临近，红利低波ETF以高股息筑牢组合安全垫，红利低波策略成稳健避风港，南方基金旗下红利低波50ETF南方(515450)跟踪标的指数标普中国A股大盘红利低波50指数(SPCLLHCP.SPI)昨日涨0.71%**

中报窗口临近，红利低波ETF以高股息筑牢组合安全垫，红利低波策略成稳健避风港，南方基金旗下红利低，风险偏好变化

风险偏好 · 偏多 · 长端 · 相关合约 T, TL · 文本置信指标 3/5

中报窗口临近，红利低波ETF以高股息筑牢组合安全垫，红利低波策略成稳健避风港，南方基金旗下红利低若引发避险需求，可能支撑利率债配置。
...
财联社8月11日早间新闻精选，财政扩张相关信息

财政政策 · 偏空 · 长端 · 相关合约 T, TL · 文本置信指标 3/5

财联社8月11日早间新闻精选可能带来债券供给或增长预期扰动，长端期货更敏感。
...
- 历史非零评分记录 188 个，次日方向命中率 59.6%，5日方向命中率 53.3%（n=184）。
...
| 数据类别 | 来源 |
|---|---|
| 国债期货 | `akshare_sina_main_daily:T0, akshare_sina_main_daily:TF0, akshare_sina_main_daily:TL0, akshare_sina_main_daily:TS0` |
| 收益率曲线 | `akshare_bond_china_close_return:2026-08-11` |
| 资金利率 | `akshare_chinamoney_repo_rate:2026-08-11, tushare_repo_daily:20260811, tushare_shibor:20260811` |
| 公开市场操作 | `tushare_news_cls:2026-08-11` |
| 政策/新闻 | `tushare_news_cls:2026-08-11` |
