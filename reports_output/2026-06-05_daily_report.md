# 国债期货日报

**2026-06-05** · 收盘观察 · 偏空

> 基于当日已入库行情与公开数据整理。宏观指标采用当时已发布的数据期。

> 历史重建版：使用当前规则重算，不代表当日实际发布的判断。

## 当日简评

国债期货各品种收跌。TS 表现相对最强，日收益 -0.010%；TL 为 -0.271%。

10年国债收益率为 1.721%，较上一观测日变化 +1.3 bp。FDR007 为 1.380%，变化 +3.0 bp。央行公开市场当日净回笼 1,230 亿元。

模型中的方向性得分来自以下判断。10Y 收益率显著上行（60日分位 95%），压制国债期货。FDR007 上行，银行间资金面边际收紧。公开市场净回笼 1230 亿元，资金面压力偏空。期货下跌且成交活跃度提高，量价配合偏消极。政策与新闻文本信号整体偏空。


模型评分 **-6.00** · 规则置信指标 **100/100**

这个指标反映评分输入的覆盖程度和方向一致性，不能解读为上涨概率。

### 关键数字

| 指标 | 当前 | 1日变化 | 5日变化 | 近20次观测分位 |
|---|---:|---:|---:|---:|
| 10Y国债收益率 | 1.721% | +1.3 bp | +1.2 bp | 35% |
| FDR007 | 1.380% | +3.0 bp | +0.0 bp | 95% |
| 期货平均收益 | -0.088% | -0.088% | +0.020% | 15% |
| 总成交量 | 365,259 | +22.8% | +6.5% | 80% |
| 总持仓量 | 901,838 | -0.3% | +6.1% | 95% |

*收益率与资金利率的变化以 bp 计，1 bp 等于 0.01 个百分点。分位只使用已有同口径观测，不足20条时按实际样本计算；无法比较时注明原因。期货5日栏为五次观测的复合收益，成交量和持仓量5日栏为相对五个交易观测前的变化率。*

## 期货表现

![国债期货累计表现](assets/2026-06-05_futures_20d.svg)

*以窗口首个观测日前的100为基准，逐日复合各品种收益率；横轴列出观测日期。*

| 合约 | 日收益 | 持仓变化 | 成交量/均值 | 持仓量/均值 | 量价状态 |
|---|---:|---:|---:|---:|---|
| TS | -0.010% | +1.00% | 1.04x | 1.20x | 价跌增仓 |
| TF | -0.023% | +0.78% | 1.23x | 1.22x | 价跌增仓 |
| T | -0.046% | -0.62% | 1.00x | 1.33x | 价跌减仓 |
| TL | -0.271% | -1.46% | 1.18x | 1.37x | 价跌减仓 |

*均值取当前20次观测窗口内、当日之前的数据。量价状态只描述价格与持仓的同日变化，不据此判断多空开仓。TS、TF、T、TL 分别对应2年、5年、10年和30年期国债期货品种。*

<details>
<summary>查看期货收盘价和成交持仓明细</summary>

| 合约 | 收盘价 | 日收益率 | 成交量 | 持仓量 |
|---|---:|---:|---:|---:|
| TS | 102.630 | -0.0097% | 44,962 | 85,476 |
| TF | 106.425 | -0.0235% | 106,748 | 229,883 |
| T | 109.225 | -0.0458% | 108,345 | 393,856 |
| TL | 113.960 | -0.2713% | 105,204 | 192,623 |

</details>

## 利率与资金

### 国债收益率曲线

![当日与前一观测日国债收益率曲线](assets/2026-06-05_yield_curve.svg)

| 结构 | 当前 | 1日变化 | 5日变化 |
|---|---:|---:|---:|
| 2s10s | +48.3 bp | +1.3 bp | -1.2 bp |
| 10s30s | +48.3 bp | -0.0 bp | -1.3 bp |
| 2s5s10s蝶式 | +10.9 bp | +0.1 bp | +2.4 bp |

*图中期限按类别等距排列。2s10s 为10年减2年收益率，10s30s 为30年减10年收益率。蝶式采用2年减两倍5年再加10年收益率，均以 bp 表示。*

<details>
<summary>查看各期限收益率与中债、CFETS 核验</summary>

| 期限 | 收益率 | 较上一观测日 |
|---|---:|---:|
| 1Y | 1.177% | +1.7 bp |
| 2Y | 1.238% | +0.0 bp |
| 5Y | 1.425% | +0.6 bp |
| 10Y | 1.721% | +1.3 bp |
| 30Y | 2.204% | +1.3 bp |

本期缺少两家发布方的可比曲线。

</details>

### 资金利率

![资金利率历史观测](assets/2026-06-05_funding_20d.svg)

| 指标 | 利率 | 较上一观测日 |
|---|---:|---:|
| DR001 | 1.341% | 无前一观测日同口径记录 |
| DR007 | 1.382% | 无前一观测日同口径记录 |
| FDR001 | 1.330% | +0.0 bp |
| FDR007 | 1.380% | +3.0 bp |
| FR007 | 1.390% | +1.0 bp |
| R007 | 1.391% | 无前一观测日同口径记录 |
| SHIBOR_7D | 1.382% | +3.4 bp |
| SHIBOR_ON | 1.324% | +0.8 bp |

*FDR 与 FR 是定盘利率，与 DR、R 加权成交利率口径不同。*

### 公开市场操作

金额单位为亿元。

| 类型 | 期限 | 投放 | 到期 | 净投放 |
|---|---:|---:|---:|---:|
| 逆回购 | 7 天 | 0 | 1,230 | -1,230 |

净投放为负数时表示净回笼。

## 宏观与后续观察

2026-05 的 制造业PMI 为 50.0，较前期下降 0.3，高于50荣枯线 0.0。

2026-04 的 CPI 为 1.2%，较前期上升 0.2。

2026-04 的 PPI 为 2.8%，较前期上升 2.3。


![CPI、PPI 与制造业 PMI 历史](assets/2026-06-05_macro_history.svg)

<details>
<summary>查看宏观数据期与历史明细</summary>

| 指标 | 数值 | 数据期 |
|---|---:|---|
| CPI 同比 | 1.20% | 2026-04 |
| LPR 1年期 | 3.00% | 2026-05-20 |
| LPR 5年期以上 | 3.50% | 2026-05-20 |
| 制造业 PMI | 50.00 | 2026-05 |
| PPI 同比 | 2.80% | 2026-04 |

| 数据期 | CPI 同比 | PPI 同比 | 制造业 PMI |
|---|---:|---:|---:|
| 2026-05 | 截至当日未取得发布值 | 截至当日未取得发布值 | 50.0 |
| 2026-04 | 1.2% | 2.8% | 50.3 |
| 2026-03 | 1.0% | 0.5% | 50.4 |
| 2026-02 | 1.3% | -0.9% | 49.0 |
| 2026-01 | 0.2% | -1.4% | 49.3 |
| 2025-12 | 0.8% | -1.9% | 50.1 |

国家统计局历史数据按日报日期截断。各指标发布时间不同，未取得的当期发布值不以之后发布的数据回填。

</details>

本期未取得完整国债发行日历，发行规模和净融资暂不列数。

下一次更新继续观察资金利率、长端收益率和期货持仓是否同向变化。发行日历恢复后，再补充已公告招标安排。

### 政策与新闻

**财联社6月5日电，美国5月就业数据公布后，交易员加深了美联储进一步收紧货币政策的**

财联社6月5日电，美国5月就业数据公布后，交易员加深了美联储进一步收紧货币政策的，货币或流动性信号偏紧

货币政策 · 偏空 · 全曲线 · 相关合约 TS, TF, T, TL · 文本置信指标 4/5

财联社6月5日电，美国5月就业数据公布后，交易员加深了美联储进一步收紧货币政策的可能推高资金利率或收紧预期，国债期货面临压力。

**国办：省级和计划单列市人民政府加强对本地区政府投资基金的统筹管理 已有同类基金的原则上不得新设**

国办：省级和计划单列市人民政府加强对本地区政府投资基金的统筹管理 已有同类基金的原则上不得新设，财政扩张相关信息

财政政策 · 偏空 · 长端 · 相关合约 T, TL · 文本置信指标 3/5

国办：省级和计划单列市人民政府加强对本地区政府投资基金的统筹管理 已有同类基金的原则上不得新设可能带来债券供给或增长预期扰动，长端期货更敏感。

**财联社6月5日午间新闻精选**

财联社6月5日午间新闻精选，财政扩张相关信息

财政政策 · 偏空 · 长端 · 相关合约 T, TL · 文本置信指标 3/5

财联社6月5日午间新闻精选可能带来债券供给或增长预期扰动，长端期货更敏感。

**医保个人账户跨省共济来了 近亲属可用**

医保个人账户跨省共济来了 近亲属可用，财政扩张相关信息

财政政策 · 偏空 · 长端 · 相关合约 T, TL · 文本置信指标 3/5

医保个人账户跨省共济来了 近亲属可用可能带来债券供给或增长预期扰动，长端期货更敏感。

**财联社6月5日早间新闻精选**

财联社6月5日早间新闻精选，资金面信息需结合价格变化判断

资金流动性 · 中性 · 全曲线 · 相关合约 TS, TF, T, TL · 文本置信指标 3/5

财联社6月5日早间新闻精选影响持仓成本和短端预期，方向取决于净投放规模与资金利率表现。

**投资日历：周五资本市场大事提醒**

投资日历：周五资本市场大事提醒，资金面信息需结合价格变化判断

资金流动性 · 中性 · 全曲线 · 相关合约 TS, TF, T, TL · 文本置信指标 3/5

投资日历：周五资本市场大事提醒影响持仓成本和短端预期，方向取决于净投放规模与资金利率表现。

**周五（6月5日）重点关注财经事件和经济数据**

周五（6月5日）重点关注财经事件和经济数据，海外利率扰动

海外利率 · 中性 · 长端 · 相关合约 T, TL · 文本置信指标 2/5

周五（6月5日）重点关注财经事件和经济数据通过全球期限溢价影响国内长端，需要结合国内资金面确认。


---

## 方法与数据附录

本报告用于整理当日市场信息。评分阈值为 +2 和 -2，区间内保留中性判断。缺少新闻、发行日历等输入时，相关解释范围也会收窄。

<details>
<summary>展开评分依据与历史检验</summary>

| 维度 | 权重 | 分数 | 数据状态 | 判断依据 |
|---|---:|---:|---|---|
| 利率方向 | 2.0 | -2.00 | 可用 | 10Y 收益率显著上行（60日分位 95%），压制国债期货。 |
| 曲线形态 | 0.5 | +0.00 | 可用 | 10Y-2Y 利差处于中性区间。 |
| 资金面 | 1.0 | -1.00 | 可用 | FDR007 上行，银行间资金面边际收紧。 |
| 公开市场操作 | 1.0 | -1.00 | 可用 | 公开市场净回笼 1230 亿元，资金面压力偏空。 |
| 期货量价 | 1.0 | -1.00 | 可用 | 期货下跌且成交活跃度提高，量价配合偏消极。 |
| 文本信号 | 1.0 | -1.00 | 可用 | 政策与新闻文本信号整体偏空。 |
| 宏观基本面 | 0.5 | +0.00 | 可用 | 制造业 PMI 50.0 处于荣枯线附近，宏观基本面中性。 |

### 风险边界

- 30Y-10Y 曲线偏陡，可能反映超长端供给或期限溢价扰动。
- 规则评分用于研究观察，不提供价格预测或个性化交易建议。

### 历史方向检验

- 历史非零评分记录 147 个，次日方向命中率 59.2%，5日方向命中率 56.9%（n=144）。
- 该检验未扣除交易成本，也未做样本外验证，只用于监测规则是否失效。

这里按评分正负划分方向，包含尚未达到 ±2 阈值的记录。历史评分受数据覆盖和规则版本影响，不等同于当时实际发布的判断。

### 原始特征

| 分组 | 指标 | 数值 |
|---|---|---:|
| 利率 | 10Y 收益率变化 | 0.0132 |
| 利率 | 30Y 收益率变化 | 0.0130 |
| 利率 | 10Y-2Y 利差 | 0.4831 |
| 利率 | 30Y-10Y 利差 | 0.4833 |
| 资金面 | 资金锚名称 | FDR007 |
| 资金面 | 资金锚水平 | 1.3800 |
| 资金面 | 资金锚变化 | 0.0300 |
| 资金面 | 7天回购分层利差 | 0.0100 |
| 资金面 | Shibor 7D-资金锚 | 0.0020 |
| 资金面 | 可用资金利率 | DR001, DR007, FDR001, FDR007, FR007, R007, SHIBOR_7D, SHIBOR_ON |
| 公开市场操作 | 公开市场净投放 | -1230.0000 |
| 公开市场操作 | 公开市场操作利率 | 原始操作记录未提供该字段 |
| 公开市场操作 | 公开市场操作记录数 | 1 |
| 期货量价 | 期货平均日收益率 | -0.0009 |
| 期货量价 | 成交活跃度变化 | 0.2288 |
| 期货量价 | 覆盖合约数量 | 4 |
| 文本 | 文本情绪均值 | -0.4444 |
| 文本 | 文本信号数量 | 9 |
| 宏观基本面 | LPR 1年期 | 3.0000 |
| 宏观基本面 | LPR 5年期以上 | 3.5000 |
| 宏观基本面 | CPI 同比 | 1.2000 |
| 宏观基本面 | PPI 同比 | 2.8000 |
| 宏观基本面 | 制造业 PMI | 50.0000 |
| 宏观基本面 | 宏观指标数量 | 5 |

</details>

<details>
<summary>展开数据来源、缺项与运行记录</summary>

本期共记录 50 条数据，其中期货 4 条、收益率 5 条、资金利率 8 条。运行状态为 success。

### 数据来源

| 数据类别 | 来源 |
|---|---|
| 国债期货 | `akshare_cffex_daily:20260605` |
| 收益率曲线 | `tushare_yc_cb:20260605` |
| 资金利率 | `akshare_chinamoney_repo_rate:2026-06-05, tushare_repo_daily:20260605, tushare_shibor:20260605` |
| 公开市场操作 | `tushare_news_cls:2026-06-05` |
| 政策/新闻 | `tushare_news_cls:2026-06-05` |
| 宏观基本面 | `akshare_chinamoney_lpr:2026-05-20, nbs_official_release:2026-05-11:https://www.stats.gov.cn/sj/zxfb/202605/t20260511_1963658.html, nbs_official_release:2026-05-11:https://www.stats.gov.cn/sj/zxfb/202605/t20260511_1963659.html, nbs_official_release:2026-05-31:https://www.stats.gov.cn/sj/zxfb/202605/t20260531_1963824.html` |

公开市场操作采用以下原始记录。

- 投资日历：周五资本市场大事提醒

### 数据状态

**国债收益率 · 正常**

观测日 未记录，共 5 条。

`bond_yields`

```text
历史数据重建；保留已有真实记录
```

**跨市场数据 · 不可用**

观测日 未记录，共 0 条。

`cross_market`

```text
未取得经验证的历史数据，不作估算
```

**可交割券、基差与 IRR · 不可用**

观测日 未记录，共 0 条。

`ctd_basis_irr`

```text
未取得经验证的历史数据，不作估算
```

**同业存单与 IRS · 不可用**

观测日 未记录，共 0 条。

`funding_ncd_irs`

```text
未取得经验证的历史数据，不作估算
```

**资金利率 · 正常**

观测日 未记录，共 8 条。

`funding_rates`

```text
历史数据重建；保留已有真实记录
```

**国债期货 · 正常**

观测日 未记录，共 4 条。

`futures_quotes`

```text
历史数据重建；保留已有真实记录
```

**国家统计局历史 · 正常**

观测日 2026-05-31，共 18 条。

`macro_history`

```text
历史数据重建；保留已有真实记录
```

**最新宏观指标 · 正常**

观测日 未记录，共 5 条。

`macro_indicators`

```text
历史数据重建；保留已有真实记录
```

**公开市场操作 · 部分可用**

观测日 2026-06-05，共 1 条。

`open_market_operations`

```text
已剔除周度汇总或未来操作计划的误采记录；仅保留原有当日记录，不将未知操作补为零
```

**政策新闻 · 正常**

观测日 未记录，共 9 条。

`policy_news`

```text
历史数据重建；保留已有真实记录
```

**国债招标结果 · 不可用**

观测日 未记录，共 0 条。

`treasury_auction_results`

```text
未取得经验证的历史数据，不作估算
```

**国债发行日历 · 不可用**

观测日 未记录，共 0 条。

`treasury_issuance_calendar`

```text
历史源未补齐；不代表当日无事件，不用现值填补
```

**双源曲线核验 · 不可用**

观测日 未记录，共 0 条。

`yield_curve_comparisons`

```text
历史源未补齐；不代表当日无事件，不用现值填补
```

### 运行记录

```text
database: data/bond_futures_monitor.db
futures_quotes: 4
bond_yields: 5
funding_rates: 8
open_market_operations: 1
policy_news: 9
macro_indicators: 5
macro_history: 18
treasury_issuance_calendar: 0
yield_curve_comparisons: 0
ai_text_signals: 9
daily_features: 1
daily_market_signals: 1
run_status: success
```

</details>
