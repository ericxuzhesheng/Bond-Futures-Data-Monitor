...
- 历史非零评分记录 53 个，次日方向命中率 52.8%，5日方向命中率 46.0%（n=50）。
- 该检验未扣除交易成本，也未做样本外验证，只用于监测规则是否失效。

这里按评分正负划分方向，包含尚未达到 ±2 阈值的记录。该结果采用历史存储评分，未统一重算规则版本。
...
观测日 2026-09-01，共 0 条。

`policy_news`

```text
AkShare 无目标日历史电报，且 Tushare news 无权限
```
